    ...
    private ResponseDto handleRequest(RequestDto request) {
        ...
        switch (request.getController()) {
            ...
            case "Maintenance":
                ...
                // Broadcast when maintenance is added
                if ("add".equals(request.getRequest()) && response.isSuccess()) {
                    MaintenanceResponseDto maintenance = gson.fromJson(response.getData(), MaintenanceResponseDto.class);
                    String notification = "New maintenance added: " + maintenance.getDescription() +
                            " for car " + maintenance.getCar().getMake() + " " + maintenance.getCar().getModel();
                    System.out.println("[ClientHandler] Maintenance added, broadcasting: " + notification);
                    server.broadcast(notification);
                }
                break;
